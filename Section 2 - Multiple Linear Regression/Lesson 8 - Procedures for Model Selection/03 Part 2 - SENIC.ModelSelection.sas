...
* Also change 'MedSchool' to 0 if it is a 2. This will make MedSchool an indicator variable;
...
PROC CORR DATA=senic NOPROB NOSIMPLE;
  ...
  RUN;
/* Note that Census and NumBeds are highly correlated */

/* Perform the various model selection procedures */
PROC REG DATA=senic;
 fullmodel: MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch  Reg1 Reg2 Reg3  / VIF;
 forward:   MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch {Reg1 Reg2 Reg3} / VIF SELECTION=FORWARD  DETAILS=SUMMARY;
 backward:  MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch {Reg1 Reg2 Reg3} / VIF SELECTION=BACKWARD DETAILS=SUMMARY;
 stepwise:  MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch {Reg1 Reg2 Reg3} / VIF SELECTION=STEPWISE DETAILS=SUMMARY;
 AdjR2:     MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch  Reg1 Reg2 Reg3  / VIF SELECTION=ADJRSQ   DETAILS=SUMMARY BEST=6 AIC SBC ADJRSQ;
 MallowsCp: MODEL InfRisk = Stay Age CulRatio XRay NumBeds Census Nurses Services MedSch  Reg1 Reg2 Reg3  / VIF SELECTION=CP       DETAILS=SUMMARY BEST=6 AIC SBC ADJRSQ;
 RUN;

/* get the diagnostic plots for the candidate models */
PROC REG DATA=senic;
